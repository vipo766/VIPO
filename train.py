...
from models.actor_critic import ActorProb, Critic, ValNet
...
def train(args=get_args()):
    ...
    true_value_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims)
    model_value_backbone = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims)
    true_valnet = ValNet(true_value_backbone, args.device)  # valnet for learning dynamics
    model_valnet = ValNet(model_value_backbone, args.device)  # valnet for learning dynamics
    
    value_backbone1 = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims)
    value_backbone2 = MLP(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims)
    valnet1 = ValNet(value_backbone1, args.device)  # valnet for learning the policy
    valnet2 = ValNet(value_backbone2, args.device)  # valnet for learning the policy
    valnet_optim1 = torch.optim.Adam(valnet1.parameters(), lr=args.valnet_lr)
    valnet_optim2 = torch.optim.Adam(valnet2.parameters(), lr=args.valnet_lr)
    ...
    # create dynamics
    load_dynamics_model = args.load_model
    model_file_path = os.path.join(args.load_dynamics_path, args.task)
    ...
    log_dirs = make_log_dirs(
        ...
        record_params=["penalty_coef", "rollout_length", "phi", "real_ratio"]
    )
